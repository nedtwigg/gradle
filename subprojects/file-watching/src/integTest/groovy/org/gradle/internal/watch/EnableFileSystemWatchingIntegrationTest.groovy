...
class EnableFileSystemWatchingIntegrationTest extends AbstractFileSystemWatchingIntegrationTest {
    ...
    @Unroll
    def "can set to #watchMode via init script in user home"() {
        buildFile << """
            apply plugin: "java"
        """
        ...
        where:
        watchMode          | expectedEnabledMessage       | expectedActiveMessage
        WatchMode.ENABLED  | ENABLED_IF_AVAILABLE_MESSAGE | ACTIVE_MESSAGE
        WatchMode.DEFAULT  | ENABLED_IF_AVAILABLE_MESSAGE | ACTIVE_MESSAGE
        WatchMode.DISABLED | ENABLED_IF_AVAILABLE_MESSAGE | ACTIVE_MESSAGE
    }
}
